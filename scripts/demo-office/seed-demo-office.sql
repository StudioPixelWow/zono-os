-- ============================================================================
-- ZONO — REALISTIC DEMO OFFICE DATASET  ·  seed  ·  marker = zono_demo_office_v1
-- ----------------------------------------------------------------------------
...
--   OWNER USER : 18f0ba4d-79e5-46d0-af5c-76980cd74217  (tal.zatelman@gmail.com)
--   MARKER     : jsonb {"demo_seed":"zono_demo_office_v1"} where a jsonb column
--                exists; the token "[zono_demo_office_v1]" / "demo:zdo1" / stable
--                md5('zdo1:<type>:<n>')::uuid ids elsewhere.
--
...
-- Cleanup: scripts/demo-office/cleanup-demo-office.sql
-- ============================================================================
...
declare
  ...
  u uuid := '18f0ba4d-79e5-46d0-af5c-76980cd74217';
...
begin
  for n in 1..14 loop
    ...
    values (sid, o, u, fn[n], '05'||(20000000 + n*137)::text, 'seller'||n||'@demo.zono.test',
       cities[1 + (n % 8)], '[zono_demo_office_v1] מוכר הדגמה',
       (1500000 + n*180000), (1400000 + n*170000),
       (n % 3 <> 0), (n % 4 <> 0), (n % 2 = 0), (n % 3 = 0))
    on conflict (id) do nothing;
  end loop;
  for n in 1..20 loop
    pid := md5('zdo1:prop:'||n)::uuid;
    ...
    values (
      pid, o, u, u, u, u, (case when n<=14 then md5('zdo1:seller:'||n)::uuid else null end),
  ...
  for n in 1..14 loop
    ...
    values (md5('zdo1:ps:'||n)::uuid, o, md5('zdo1:prop:'||n)::uuid, md5('zdo1:seller:'||n)::uuid, 'owner', true, true, 'active', mk)
    on conflict (id) do nothing;
  end loop;
end $$;
...
begin
  for n in 1..35 loop
    ...
    values (md5('zdo1:buyer:'||n)::uuid, o, u, bnames[n], '05'||(30000000+n*211)::text, 'buyer'||n||'@demo.zono.test',
      ...
      jsonb_build_object('demo_seed','zono_demo_office_v1','note','קונה הדגמה'))
    on conflict (id) do nothing;
  end loop;
  for n in 1..50 loop
    ...
    values (md5('zdo1:lead:'||n)::uuid, o,
      ...
      '[zono_demo_office_v1] פנייה מהדגמה',
      (case when n%9=0 then 88 when n%5=0 then 70 else (30+(n%40)) end),
      (case when n%4=0 then md5('zdo1:prop:'||(1+(n%20)))::uuid else null end),
...
do $$
declare o uuid := '1a1e7da6-bb85-420a-978a-7deb8c35e63f'; u uuid := '18f0ba4d-79e5-46d0-af5c-76980cd74217';
begin
  ...
  select md5('zdo1:mtg:'||k)::uuid, o, u, mt::meeting_type, ms::meeting_status, ttl,
     '[zono_demo_office_v1]', st, st + '60 min'::interval, prop, buy, sel, 'demo:zdo1', cmp, oc
  from (values
    (1,'meeting','confirmed','פגישת קונה — אלון מור', ((current_date)::text||' 10:00')::timestamp at time zone 'Asia/Jerusalem', md5('zdo1:prop:1')::uuid, md5('zdo1:buyer:1')::uuid, null::uuid, null::timestamptz, null::text),
    (2,'viewing','confirmed','ביקור בנכס — פנטהאוז', ((current_date)::text||' 14:00')::timestamp at time zone 'Asia/Jerusalem', md5('zdo1:prop:8')::uuid, md5('zdo1:buyer:3')::uuid, null, null, null),
    (3,'call','scheduled','שיחת מוכר — משה דהן', ((current_date)::text||' 16:00')::timestamp at time zone 'Asia/Jerusalem', md5('zdo1:prop:3')::uuid, null, md5('zdo1:seller:3')::uuid, null, null),
    (4,'viewing','scheduled','ביקור אחה״צ — דירת גן', ((current_date)::text||' 17:30')::timestamp at time zone 'Asia/Jerusalem', md5('zdo1:prop:9')::uuid, md5('zdo1:buyer:6')::uuid, null, null, null),
    (5,'viewing','scheduled','ביקור מקביל — דירת 4 חד', ((current_date)::text||' 14:00')::timestamp at time zone 'Asia/Jerusalem', md5('zdo1:prop:2')::uuid, md5('zdo1:buyer:9')::uuid, null, null, null),
    (6,'viewing','confirmed','ביקור — קוטג', ((current_date+1)::text||' 11:00')::timestamp at time zone 'Asia/Jerusalem', md5('zdo1:prop:7')::uuid, md5('zdo1:buyer:12')::uuid, null, null, null),
    (7,'viewing','scheduled','ביקור — דירת 5 חד', ((current_date+2)::text||' 18:00')::timestamp at time zone 'Asia/Jerusalem', md5('zdo1:prop:6')::uuid, md5('zdo1:buyer:15')::uuid, null, null, null),
    (8,'meeting','confirmed','פגישת אסטרטגיה — מוכר', ((current_date+3)::text||' 09:30')::timestamp at time zone 'Asia/Jerusalem', md5('zdo1:prop:4')::uuid, null, md5('zdo1:seller:4')::uuid, null, null),
    (9,'viewing','scheduled','ביקור — פנטהאוז דופלקס', ((current_date+4)::text||' 17:00')::timestamp at time zone 'Asia/Jerusalem', md5('zdo1:prop:10')::uuid, md5('zdo1:buyer:20')::uuid, null, null, null),
    (10,'signing','scheduled','חתימת חוזה', ((current_date+5)::text||' 12:00')::timestamp at time zone 'Asia/Jerusalem', md5('zdo1:prop:11')::uuid, md5('zdo1:buyer:2')::uuid, null, null, null),
    (11,'viewing','completed','ביקור שהושלם', ((current_date-2)::text||' 16:00')::timestamp at time zone 'Asia/Jerusalem', md5('zdo1:prop:9')::uuid, md5('zdo1:buyer:4')::uuid, null, ((current_date-2)::text||' 17:00')::timestamp at time zone 'Asia/Jerusalem','עניין גבוה'),
    (12,'viewing','completed','ביקור שהושלם', ((current_date-4)::text||' 11:00')::timestamp at time zone 'Asia/Jerusalem', md5('zdo1:prop:9')::uuid, md5('zdo1:buyer:7')::uuid, null, ((current_date-4)::text||' 12:00')::timestamp at time zone 'Asia/Jerusalem','לא מתאים'),
    (13,'viewing','completed','ביקור שהושלם', ((current_date-6)::text||' 15:00')::timestamp at time zone 'Asia/Jerusalem', md5('zdo1:prop:2')::uuid, md5('zdo1:buyer:11')::uuid, null, ((current_date-6)::text||' 16:00')::timestamp at time zone 'Asia/Jerusalem','עניין בינוני'),
    (14,'viewing','completed','ביקור שהושלם', ((current_date-7)::text||' 10:00')::timestamp at time zone 'Asia/Jerusalem', md5('zdo1:prop:10')::uuid, md5('zdo1:buyer:14')::uuid, null, ((current_date-7)::text||' 11:00')::timestamp at time zone 'Asia/Jerusalem','הוגשה הצעה'),
    (15,'viewing','cancelled','ביקור שבוטל', ((current_date-1)::text||' 13:00')::timestamp at time zone 'Asia/Jerusalem', md5('zdo1:prop:14')::uuid, md5('zdo1:buyer:18')::uuid, null, null, null),
    (16,'valuation','completed','הערכת שווי', ((current_date-3)::text||' 09:00')::timestamp at time zone 'Asia/Jerusalem', md5('zdo1:prop:5')::uuid, null, md5('zdo1:seller:5')::uuid, ((current_date-3)::text||' 10:00')::timestamp at time zone 'Asia/Jerusalem','בוצעה')
  ...
  select md5('zdo1:task:'||n)::uuid, o, u, u,
     ...
     '[zono_demo_office_v1] משימת הדגמה',
     ...
     (case when n%4=0 then md5('zdo1:lead:'||(1+(n%50)))::uuid else null end),
     (case when n%4=1 then md5('zdo1:prop:'||(1+(n%20)))::uuid else null end),
     (case when n%4=2 then md5('zdo1:seller:'||(1+(n%14)))::uuid else null end),
     (case when n%4=3 then md5('zdo1:buyer:'||(1+(n%35)))::uuid else null end),
     'demo:zdo1:task:'||n,
...
begin
  ...
  select md5('zdo1:deal:'||k)::uuid, o, u, ttl, 'sale'::deal_type, stg::deal_stage, sts::deal_status,
     val, (val*0.02)::bigint, 2.0, prob, md5('zdo1:buyer:'||b)::uuid, md5('zdo1:seller:'||s)::uuid, md5('zdo1:prop:'||p)::uuid, ecd, cl, lr
  from (values
  ...
  select md5('zdo1:dp:'||k)::uuid, o, md5('zdo1:deal:'||k)::uuid, md5('zdo1:buyer:'||b)::uuid, md5('zdo1:seller:'||s)::uuid,
     md5('zdo1:prop:'||p)::uuid, u, dstage, health, risk, velo, prob, val, (val*0.02)::bigint, ecd, blocker, nba, st, 'חיפה', mk, now()
  from (values
  ...
  select md5('zdo1:dj:'||k||':'||stg)::uuid, o, md5('zdo1:dp:'||k)::uuid, stg, ent, ex,
     round(extract(epoch from (coalesce(ex,now())-ent))/3600)::int, u, '[zdo1]'
  from (values
  ...
  values
    (md5('zdo1:do:8')::uuid, o, md5('zdo1:dp:8')::uuid, 'price', 'high', false, u, 'הקונה חרג מהתקציב ב-8%','להציע נכס חלופי בטווח המחיר'),
    (md5('zdo1:do:3')::uuid, o, md5('zdo1:dp:3')::uuid, 'timing', 'medium', false, u, 'הקונה מתלבט לגבי עיתוי','לתאם פגישת החלטה')
  on conflict (id) do nothing;
end $$;

-- ── Stage 5 — activity history + marketing (non-claimable) + support + notifs ─
do $$
declare o uuid := '1a1e7da6-bb85-420a-978a-7deb8c35e63f'; u uuid := '18f0ba4d-79e5-46d0-af5c-76980cd74217'; mk jsonb := '{"demo_seed":"zono_demo_office_v1"}'; n int;
begin
  for n in 1..40 loop
    ...
    values (md5('zdo1:ae:created:'||n)::uuid, o, u, 'lead.created', 'lead', md5('zdo1:lead:'||n)::uuid::text, 'ליד חדש נקלט', 'system', now()-((n%14+1)||' days')::interval, mk)
    on conflict (id) do nothing;
    if n % 3 <> 0 then
      ...
      values (md5('zdo1:ae:contact:'||n)::uuid, o, u, 'lead.contacted', 'lead', md5('zdo1:lead:'||n)::uuid::text, 'יצירת קשר עם ליד', 'whatsapp', 'outbound',
  ...
  select md5('zdo1:ae:click:'||g)::uuid, o, 'property.contact_clicked', 'property', md5('zdo1:prop:'||((g%9)+1))::uuid::text, 'לחיצת יצירת קשר על נכס', now()-((g%6)||' days')::interval, mk
  ...
  values
   (md5('zdo1:camp:1')::uuid, o, md5('zdo1:prop:2')::uuid, 'קמפיין דירת 4 חד'' חיפה', 'active', now()-'5 days'::interval, mk),
   (md5('zdo1:camp:2')::uuid, o, md5('zdo1:prop:8')::uuid, 'קמפיין פנטהאוז', 'active', now()-'2 days'::interval, mk),
   (md5('zdo1:camp:3')::uuid, o, md5('zdo1:prop:3')::uuid, 'קמפיין דירת 3 חד''', 'completed', now()-'20 days'::interval, mk)
  on conflict (id) do nothing;
  ...
  values
   (md5('zdo1:post:1')::uuid, o, md5('zdo1:camp:1')::uuid, md5('zdo1:prop:2')::uuid, 'facebook_groups', 'נכס חדש למכירה בחיפה', 'published', 'published', true, now()-'4 days'::interval, null, null, mk),
   (md5('zdo1:post:2')::uuid, o, md5('zdo1:camp:1')::uuid, md5('zdo1:prop:2')::uuid, 'facebook_groups', 'עדכון מחיר', 'published', 'published', true, now()-'2 days'::interval, null, null, mk),
   (md5('zdo1:post:3')::uuid, o, md5('zdo1:camp:2')::uuid, md5('zdo1:prop:8')::uuid, 'facebook_groups', 'פנטהאוז עם נוף', 'published', 'published', true, now()-'1 day'::interval, null, null, mk),
   (md5('zdo1:post:4')::uuid, o, md5('zdo1:camp:2')::uuid, md5('zdo1:prop:8')::uuid, 'facebook_groups', 'פרסום שנכשל', 'dead_letter', 'failed', true, null, 'קבוצה דחתה את הפרסום', null, mk)
  on conflict (id) do nothing;

  insert into public.marketing_plans (id, org_id, property_id, created_by, status, plan_json)
  values
   (md5('zdo1:mp:1')::uuid, o, md5('zdo1:prop:6')::uuid, u, 'draft', jsonb_build_object('demo_seed','zono_demo_office_v1','items',3)),
   (md5('zdo1:mp:2')::uuid, o, md5('zdo1:prop:2')::uuid, u, 'active', jsonb_build_object('demo_seed','zono_demo_office_v1','items',5)),
   (md5('zdo1:mp:3')::uuid, o, md5('zdo1:prop:10')::uuid, u, 'failed', jsonb_build_object('demo_seed','zono_demo_office_v1','items',4,'error','provider'))
  on conflict (id) do nothing;

  insert into public.support_tickets (id, org_id, user_id, subject, description, status, priority, category)
  values
   (md5('zdo1:tkt:1')::uuid, o, u, 'שאלה על חיבור WhatsApp', '[zdo1] איך מחברים מספר עסקי', 'open', 'normal', 'integration'),
   (md5('zdo1:tkt:2')::uuid, o, u, 'בקשת שינוי בדוח מוכר', '[zdo1] ממתין לפרטים מהלקוח', 'waiting_customer', 'low', 'general'),
   (md5('zdo1:tkt:3')::uuid, o, u, 'תקלה בהעלאת תמונות', '[zdo1] נפתר', 'resolved', 'normal', 'technical'),
   (md5('zdo1:tkt:4')::uuid, o, u, 'חיוב לא תקין דחוף', '[zdo1] דורש טיפול מיידי', 'open', 'urgent', 'billing')
  on conflict (id) do nothing;
  ...
  select md5('zdo1:notif:'||g)::uuid, o, u,
    ...
    '[zono_demo_office_v1]', (g%3=0), '/action-center',
    (case when g in (1,2) then md5('zdo1:lead:'||g)::uuid else null end),
    (case when g=3 then md5('zdo1:deal:1')::uuid else null end),
    (case when g in (4,5) then md5('zdo1:prop:'||g)::uuid else null end),
...
do $$
declare o uuid := '1a1e7da6-bb85-420a-978a-7deb8c35e63f'; b int; j int; prop int; pid uuid; bid uuid; st text; msc int; pr int;
  stat text[] := array['recommended','viewed','interested','viewing_requested','rejected'];
begin
  for b in 1..12 loop
    bid := md5('zdo1:bundle:'||b)::uuid;
    for j in 0..(3 + (b % 3)) loop
      prop := 1 + ((b + j) % 16);
      pid := md5('zdo1:prop:'||prop)::uuid;
      st := stat[1 + ((b + j) % 5)];
      msc := 62 + ((b*7 + j*5) % 34);
      ...
      values (md5('zdo1:cpr:'||b||':'||prop)::uuid, o, 'buyer', md5('zdo1:buyer:'||b)::uuid, pid, bid,
  ...
  insert into public.customer_property_recommendations
    (id, org_id, contact_type, contact_id, property_id, bundle_id, channel, status, match_score, price_at_send, recommended_at, responded_at)
  select md5('zdo1:cpr:hot:'||g||':'||p)::uuid, o, 'buyer', md5('zdo1:buyer:'||g)::uuid, md5('zdo1:prop:'||p)::uuid,
     md5('zdo1:bundle:hot:'||g)::uuid, 'whatsapp', 'interested', 80+g,
     (select price::int from public.properties where id=md5('zdo1:prop:'||p)::uuid), now()-'3 days'::interval, now()-'1 day'::interval

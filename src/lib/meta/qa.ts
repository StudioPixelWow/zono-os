...
{
  const has14 = META_EVENT_NAMES.length === 42; // +6.9 P6 (message.sent, message.send_review)
...
// ── Registry integrity · MVP/Extended/Excluded counts ────────────────────────
{
  ...
  check(`Registry: capability classes present (mvp=${mvp}, extended=${ext}, excluded=${exc})`, mvp === 12 && ext === 13 && exc === 2 && groupsExcluded);
}

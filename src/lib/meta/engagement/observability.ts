...
// queue-health route to bring engagement to parity with the other Meta queues.
export function evaluateEngagementHealth(input: { byStatus: Readonly<Record<string, number>>; deadLetter: number; oldestDueMs: number | null }): { healthy: boolean; backlog: number; degraded: boolean } {

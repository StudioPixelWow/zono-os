...
async function main() {
  ...
  check("D87 no periodic reconciliation file in the immediate-publish surface", !metaFiles.filter((f) => { const n = f.replace(/\\/g, "/"); return !n.includes("/reconcile/") && !n.includes("/webhooks/"); }).some((f) => /reconcil/i.test(f)));
  ...
  { let ok = true; try { const out = execSync("git status --porcelain", { cwd: ROOT, encoding: "utf8" });
      const off = out.split("\n").map((l) => l.trim().replace(/^\S+\s+/, "")).filter(Boolean).filter((f) => !(f.startsWith("src/lib/meta/") || f.startsWith("src/app/api/meta/") || f.startsWith("src/app/api/internal/meta/") || f.startsWith("src/app/(app)/meta-workspace/") || f === "package.json" || f === "scripts/check-meta-boundaries.mjs" || /supabase\/migrations\/(2026121[025]12|2026122[05]12)/.test(f)));

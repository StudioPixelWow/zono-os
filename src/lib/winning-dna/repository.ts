// ============================================================================
// Broker Winning DNA™ — MAI-9 repository (server-only).
//
...
// per-listing records the engine groups by segment × window × broker. No DNA
// math here — that lives in engine.ts (pure). Service-role scoped.
// ============================================================================
import "server-only";
import { createServiceRoleClient } from "@/lib/supabase/server";
import type { MarketAcceptanceClassification, ListingLifecycleState } from "@/lib/market-acceptance/types";
import type { WinningDNARecord } from "./types";
...
export async function gatherWinningDNARecords(organizationId: string): Promise<WinningDNARecord[]> {
...
/** Upsert computed winning-DNA rows (conflict-keyed by segment + window). */
export async function upsertWinningDNARows(rows: Record<string, unknown>[]): Promise<void> {
  if (!rows.length) return;
  const db = createServiceRoleClient() as Db;
  for (let i = 0; i < rows.length; i += 500) {
    try {
      await db
        .from("broker_winning_dna" as never)

...
export async function getHomeMapDataAction(filters: HomeMapFilters = DEFAULT_HOME_MAP_FILTERS): Promise<HomeMapData> {
  try { return await getHomeMapData(filters); }
    ...
    return { points: [], internalCount: 0, externalCount: 0, total: 0, hasGoogleKey: !!process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY, hasOperatingArea: false, areaCities: [], areaLabel: null, externalDiag: { rawActive: 0, withCoords: 0, missingCoords: 0, cityDropped: 0, shown: 0 } };
  }
}

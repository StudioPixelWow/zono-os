...
check("B6 fixture qrcode.updated (no image) → status waiting_qr", (() => { const r = normalizeWebhook({ event: "qrcode.updated", instance: instanceName({ orgId: "O", userId: "U" }), data: {} }, NOW); return r.kind === "status" && r.state === "waiting_qr"; })());
check("B6a qrcode.updated with nested base64 → kind qr carries the scannable image", (() => { const r = normalizeWebhook({ event: "qrcode.updated", instance: instanceName({ orgId: "O", userId: "U" }), data: { qrcode: { base64: "data:image/png;base64,AAA", code: "2@abc" } } }, NOW); return r.kind === "qr" && r.qr.image === "data:image/png;base64,AAA" && r.qr.raw === "2@abc" && typeof r.qr.expiresAt === "string"; })());
check("B6b qrcode.updated with flat base64 → kind qr (accepts flat shape too)", (() => { const r = normalizeWebhook({ event: "qrcode.updated", instance: instanceName({ orgId: "O", userId: "U" }), data: { base64: "data:image/png;base64,BBB" } }, NOW); return r.kind === "qr" && r.qr.image === "data:image/png;base64,BBB"; })());

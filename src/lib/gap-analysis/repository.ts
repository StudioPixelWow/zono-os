// ============================================================================
// Broker Gap Analysis™ — MAI-10 repository (server-only).
//
...
// per-listing records the engine groups by segment × window × broker. No gap
...
export async function gatherGapRecords(organizationId: string): Promise<GapRecord[]> {
...
/** Upsert computed gap-analysis rows (conflict-keyed by broker + segment + window). */
export async function upsertGapRows(rows: Record<string, unknown>[]): Promise<void> {

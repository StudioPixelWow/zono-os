...
import { buildInfraAttentionRows, buildRecommendationAttentionRows, buildTerritoryAttentionRows, buildPortalAttentionRows, buildOfficeSiteAttentionRows, buildAgentSiteAttentionRows, buildAutomationAttentionRows, buildDocumentAttentionRows, buildFinancingAttentionRows, buildReputationAttentionRows, buildCommunityAttentionRows, buildWhatsappAttentionRows, buildCommIntelAttentionRows, buildJourneyAttentionRows } from "@/lib/system/decision-signals";
...
function buildAttentionRows(orgId: string, d: OrgData): AttentionInsert[] {
  ...
  // Competitor intelligence signals — market structure attention.
  for (const s of d.competitorSignals) {
    if (!s.competitorId) continue;
    const isOpportunity = s.signalType === "competitor_losing_inventory" || s.signalType === "vulnerable_broker";
    rows.push({
      org_id: orgId, entity_type: "competitor", entity_id: s.competitorId,
      attention_score: s.signalType === "dominant_broker" ? 62 : isOpportunity ? 68 : 58,
      urgency_score: 55, impact_score: 60, confidence_score: 70,
  ...
  if (d.revenue) {
    if ((d.revenue.gapLevel === "risk" || d.revenue.gapLevel === "critical") && d.revenue.gap > 0) {
      rows.push({
        org_id: orgId, entity_type: "revenue", entity_id: "org",
        attention_score: d.revenue.gapLevel === "critical" ? 84 : 74, urgency_score: d.revenue.gapLevel === "critical" ? 80 : 68,
        impact_score: clamp(100 - d.revenue.gapScore), confidence_score: 72,
        revenue_impact_score: clamp(100 - d.revenue.gapScore), relationship_impact_score: 30, churn_impact_score: 0,
        title: `פער הכנסה ליעד · ${Math.round(d.revenue.gap).toLocaleString()}₪`, reason: d.revenue.summary ?? "ההכנסה הצפויה מתחת ליעד",
        recommended_action: "מקד פעולות בהזדמנויות ההכנסה המהירות במודיעין ההכנסות", expected_outcome: "סגירת הפער ליעד", status: "open",
      });
    }
    if (d.revenue.atRisk > 0) {
      rows.push({
        org_id: orgId, entity_type: "revenue", entity_id: "org",
        attention_score: 72, urgency_score: 70, impact_score: clamp(Math.min(100, d.revenue.atRisk / 2000)), confidence_score: 70,
        revenue_impact_score: clamp(Math.min(100, d.revenue.atRisk / 2000)), relationship_impact_score: 30, churn_impact_score: 0,
        title: `הכנסה בסיכון · ${Math.round(d.revenue.atRisk).toLocaleString()}₪`, reason: "עסקאות בסיכון בצנרת",
        recommended_action: "התערב בעסקאות בסיכון לשמירת ההכנסה", expected_outcome: "שמירת הכנסה צפויה", status: "open",
      });
    }
  }
...
function buildOpportunityRows(orgId: string, d: OrgData): OppInsert[] {
  ...
  // Marketing Intelligence — where/what/to-whom to promote.
  for (const mo of d.marketingOpps.slice(0, 8)) {
    const isProp = mo.entityType === "property" && mo.entityId;
    rows.push({
      org_id: orgId, entity_type: isProp ? "property" : "marketing", entity_id: isProp ? mo.entityId! : "org",
  ...
  // Revenue Intelligence — strong probability-weighted pipeline (revenue opportunity).
  if (d.revenue && d.revenue.forecast90 > 0) {
    rows.push({
      org_id: orgId, entity_type: "revenue", entity_id: "org",
      opportunity_score: clamp(d.revenue.gapScore), impact_score: clamp(Math.min(100, d.revenue.forecast90 / 5000)), confidence_score: 72,
      title: `צנרת הכנסות 90 יום · ${Math.round(d.revenue.forecast90).toLocaleString()}₪`,
      description: d.revenue.summary ?? "צנרת הכנסות משוקללת פעילה", recommended_action: "האץ סגירות למימוש הצנרת", status: "open",
    });
  }
...
export async function recalculateOrganizationDecisionBrain(): Promise<void> {
  ...
  const attentionRows = [
    ...
    ...(await buildTerritoryAttentionRows(orgId)),
    ...(await buildPortalAttentionRows(orgId)),
    ...(await buildOfficeSiteAttentionRows(orgId)),
    ...(await buildAgentSiteAttentionRows(orgId)),
    ...(await buildAutomationAttentionRows(orgId)),
    ...
    ...(await buildWhatsappAttentionRows(orgId)),
    ...(await buildCommIntelAttentionRows(orgId)),
    ...(await buildJourneyAttentionRows(orgId)),
  ];

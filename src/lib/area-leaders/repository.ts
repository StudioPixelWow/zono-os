// ============================================================================
// Area Leader & Market Dominance Engine™ — MAI-7 repository (server-only).
//
// Pure data access: batch-reads broker-attributed listing evidence for one org
// (external_listings broker attribution + lifecycle state/DOM/last-scan +
...
// leadership math here — that lives in engine.ts (pure). Service-role scoped.
// ============================================================================
import "server-only";
...
export async function gatherAreaLeaderRecords(organizationId: string): Promise<AreaLeaderRecord[]> {
  ...
  // 2) Lifecycle (state + dom + last-known price/city/neighborhood + last_scan_at).
  ...
  // 4) Signals → price-reduction (kept for parity; not weighted into dominance).
  ...
  // 5) Join into per-listing records.
  const out: AreaLeaderRecord[] = [];
  for (const r of ext) {
    if (!r.detected_broker_id) continue; // unattributed → ignored safely
...
/** Upsert computed area-leader rows (conflict-keyed by the full segment + window). */
export async function upsertAreaLeaderRows(rows: Record<string, unknown>[]): Promise<void> {
  if (!rows.length) return;
  const db = createServiceRoleClient() as Db;
  for (let i = 0; i < rows.length; i += 500) {
    try {
      await db
        .from("market_area_leaders" as never)
        .upsert(rows.slice(i, i + 500) as never, { onConflict: "organization_id,city,neighborhood,property_type,rooms,price_bucket,window_days" });

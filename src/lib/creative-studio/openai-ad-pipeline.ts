...
export function buildAdPrompt(spec: AdSpec, assets: AdGenAssets, correction: string): string {
  ...
  const lines = [
    ...
    "CREATIVE FREEDOM — this is a FULL luxury advertising campaign, not a card. You may and SHOULD use: complex multi-zone layouts, multiple image zones / elegant collage, premium badges, magazine-style editorial composition, a premium designed icon system for highlights, layered visual storytelling, and genuine creative risk. Do NOT simplify the layout, do NOT reduce creativity, do NOT collapse into a card, template or spec sheet. Aim for advertisements comparable to the best real-estate campaigns in the world — visual excellence is the goal.",
    kindLine,
    ...
    // ── TEXT TREATMENT — designed, magazine-grade (not minimalist) ───────────
    "TEXT TREATMENT: set the locked copy (sale label, headline, address, price, agent, phone, branding) as DESIGNED, editorial, magazine-grade typography that is part of a rich composition — confident scale, premium hierarchy, designed badges and panels. This is NOT a minimalist caption pass: a full luxury campaign carries layered, art-directed type. Avoid clutter, but do NOT strip the creative down — visual richness and creative risk are encouraged. Keep every Hebrew string exactly as locked.",

    // ── PREMIUM ICON SYSTEM (allowed & encouraged) ────────────────────────────
    "PREMIUM ICON SYSTEM: you MAY use an elegant, premium designed icon system for property highlights as part of the luxury composition — ONE consistent, sharp, minimal line-icon family integrated into the magazine layout (not a cheap spec-table row). Render icons and any numerals beautifully and cleanly; never produce corrupted glyphs or broken numbers. Icons are a designed campaign element, not a brochure facts sheet — keep them refined and secondary to the property, which remains the hero.",
    ...
    "LUXURY LAYOUT: a vertical premium poster with a rich, magazine-grade composition. A LARGE hero property photo anchors the design; you MAY use multiple image zones / elegant collage, floating glass information panels, premium badges and layered sections holding the headline, price and highlights. Sophisticated visual hierarchy and confident structure — complex and editorial is welcome, high-end magazine quality. Refined, not cluttered; rich, not bare.",
    ...
    "REFINEMENT: keep decorative line work intentional and refined (avoid random clutter), but do NOT simplify the layout or reduce the creativity — a complex, rich, art-directed luxury composition is the goal. Keep the visual hierarchy, the typography hierarchy, the agent placement and the supplied branding.",

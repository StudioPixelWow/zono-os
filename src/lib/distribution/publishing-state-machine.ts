...
export const VALID_TRANSITIONS: Record<PublishState, PublishState[]> = {
  ...
  dispatching: ["awaiting_confirmation", "published", "failed", "awaiting_reconciliation", "cancelled", "paused"],
...
export function buildIdempotencyKey(p: {
  ...
  creativeVersion?: string | null; scheduleKey?: string | null;
}): string {
  return sha256Hex([
    p.orgId, p.campaignId ?? "", p.propertyId ?? "", p.destinationId ?? p.groupId ?? "",
    p.contentHash, p.creativeVersion ?? "", p.scheduleKey ?? "",
  ].join(""));
}

// ── Emergency stop ───────────────────────────────────────────────────────────
/** True if an org-wide emergency stop is active (also enforced inside the claim fn). */
export async function isGroupsEmergencyActive(db: Db, orgId: string): Promise<boolean> {
...
export interface ClaimedPost {
  ...
  creative_output_id: string | null; creative_version: string | null;
...
export async function claimNextPost(db: Db, args: { orgId: string; userId: string; instanceId: string; leaseSeconds?: number }): Promise<ClaimedPost | null> {
  ...
  // Postgres returns the row-type; supabase surfaces it as an object (or null / empty).
 ...
 * completed_at + duration; awaiting_reconciliation clears the lease so the ambiguous
 * post is never re-claimed.
 */
export async function transitionPost(db: Db, input: TransitionInput): Promise<TransitionResult> {
  ...
  // Guarded update: only transition if the row is still in `from` (optimistic concurrency).
  ...
  if (evErr && !/duplicate key/i.test(evErr.message)) {
    // Event is best-effort audit; a duplicate callback_id is an expected no-op (dedupe).

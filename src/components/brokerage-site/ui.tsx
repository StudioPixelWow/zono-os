...
function LuxuryFallback() {
  return (
    <div className="absolute inset-0" style={{ background: "var(--site-gradient)" }} aria-hidden>
...
export function PropertyCard({
  ...
  location = null, rooms = null, area = null, matchScore = null, reason = null,
}: {
  ...
  return (
    <Link
      href={`/${base}/${slug}/property/${id}`}
      className="group flex flex-col overflow-hidden rounded-3xl border border-white/50 bg-white/70 shadow-[0_2px_10px_rgba(15,23,42,0.06)] backdrop-blur-md transition duration-300 hover:-translate-y-1 hover:shadow-[0_24px_50px_-12px_rgba(15,23,42,0.28)]"
    >
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        {badge && <span className="absolute end-3 top-3 rounded-full px-2.5 py-1 text-[10px] font-black text-white shadow-sm" style={{ background: "var(--site-gradient)" }}>{badge}</span>}
        {matchScore != null && (
          <span className="absolute start-3 top-3 inline-flex items-center gap-1 rounded-full bg-white/90 px-2.5 py-1 text-[10px] font-black shadow-sm backdrop-blur-sm" style={{ color: "var(--site-accent)" }}>
            <span className="inline-block h-1.5 w-1.5 rounded-full" style={{ background: "var(--site-accent)" }} /> {matchScore}
          </span>
        )}
      </div>
      <div className="flex flex-1 flex-col gap-1 p-4">
        ...
        <div className="mt-1 flex items-end justify-between gap-2">
          {fmt(price) ? <div className="text-[17px] font-black" style={{ color: "var(--site-accent)" }}>{fmt(price)}</div> : <span className="text-[12px] font-semibold text-slate-400">מחיר לפי פנייה</span>}

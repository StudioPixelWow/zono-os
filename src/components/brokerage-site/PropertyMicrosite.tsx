...
// One luxury property microsite rendered by BOTH the office (/ai-site) and agent
// (/ai-agent) property routes — no duplication. Cinematic hero with price/location
// overlay, key-facts strip, ONE accent intelligence language (no rainbow), AI
// summary, gallery (premium fallback), neighborhood context, premium contact +
// WhatsApp/Call, similar properties (shared luxury card), Ask-about-property.
...
export function PropertyMicrosite({
  ...
  attribution = null, askApiBase, askTitle, areaBase,
}: {
    ...
    <main className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
      ...
      <section className="relative overflow-hidden rounded-[28px] shadow-[var(--shadow-lift)]">
        <div className="relative aspect-[4/5] w-full sm:aspect-[16/9]">
          ...
          {/* top row: status + attribution */}
          <div className="absolute inset-x-0 top-0 flex items-center justify-between gap-2 p-4 sm:p-5">
            {p.status && <span className="rounded-full bg-white/90 px-3 py-1 text-[11px] font-black shadow-sm backdrop-blur-sm" style={{ color: ACCENT }}>{STATUS_HE[p.status] ?? p.status}</span>}
            {attribution && <span className="zono-glass-dark rounded-full px-3 py-1 text-[11px] font-bold text-white">{attribution}</span>}
          </div>
          {/* bottom: title + location + price */}
          <div className="absolute inset-x-0 bottom-0 p-5 text-white sm:p-8">
            <h1 className="text-2xl font-black leading-tight drop-shadow-sm sm:text-4xl">{p.title}</h1>
            {loc && <p className="mt-1 text-[14px] font-semibold text-white/85 sm:text-base">📍 {loc}</p>}
      ...
      {/* ── AI intelligence — one accent language (no rainbow) ── */}
      ...
      {/* ── Premium contact block — the obvious next step ── */}
      {(whatsapp || phone) && (
        <section className="relative mt-5 overflow-hidden rounded-[24px] p-6 text-white shadow-[var(--shadow-lift)] sm:p-7" style={{ background: GRADIENT }}>
          <div className="pointer-events-none absolute -left-14 -top-16 h-48 w-48 rounded-full bg-white/15 blur-3xl" />
          <div className="relative flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
            <div>
              <p className="text-[12px] font-bold text-white/80">מעוניינים בנכס?</p>
            ...
            <div className="flex w-full flex-wrap gap-2 sm:w-auto">
              {whatsapp && <a href={`https://wa.me/${whatsapp.replace(/\D/g, "")}?text=${waText}`} target="_blank" rel="noopener" className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-2xl bg-white px-5 py-3 text-sm font-black text-[#1a1236] shadow-lg transition hover:-translate-y-0.5 sm:flex-none">💬 תיאום ביקור</a>}

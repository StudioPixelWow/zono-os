    ...
    <main style={themeVars(branding) as CSSProperties} className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      ...
      <div className="mt-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[12px] font-bold" style={{ color: "var(--site-accent)" }}>בייצוג {branding.brokerName}</p>
        </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        {branding.phone && <a href={`tel:${branding.phone}`} className="rounded-xl px-5 py-2.5 text-sm font-bold text-white" style={{ background: "var(--site-gradient)" }}>📞 חייגו ל{branding.brokerName}</a>}
        {branding.calendarLink && <a href={branding.calendarLink} className="rounded-xl border border-slate-300 px-5 py-2.5 text-sm font-bold text-slate-700">📅 קביעת פגישה</a>}
      </div>

      {/* Ask about this property */}
      <section className="mt-6"><AskWidget slug={slug} office={branding.brokerName} apiBase="agent-site" title={`שאל את ${branding.brokerName}`} suggestions={[`ספר לי עוד על ${p.title}`, "מה יש בסביבה?", "האם המחיר תחרותי?"]} /></section>

      {/* Related */}
      {p.related.length > 0 && (
        <section className="mt-8">
          <h2 className="mb-3 text-xl font-black text-slate-800">נכסים דומים בייצוגי</h2>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">{p.related.map((x) => <PropertyCard key={x.id} slug={slug} id={x.id} title={x.title} price={x.price} image={x.image} base="ai-agent" />)}</div>
        </section>
      )}
    </main>
  );

// POST /api/meta/messaging/conversations/[id]/sync → schedule a bounded message sync (no sync provider call).
import { NextResponse } from "next/server";
import { getSessionContext } from "@/lib/auth/session";
import { resolveRoleKey } from "@/lib/auth/role";
import { requestSync } from "@/lib/meta/messaging/service";
...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const r = await requestSync(c.orgId, c.role, id);

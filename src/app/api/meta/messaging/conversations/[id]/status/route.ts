// POST /api/meta/messaging/conversations/[id]/status { status } → local, audited state.
import { NextResponse } from "next/server";
import { getSessionContext } from "@/lib/auth/session";
import { resolveRoleKey } from "@/lib/auth/role";
import { setConversationStatus } from "@/lib/meta/messaging/service";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const r = await setConversationStatus(c.orgId, c.userId, c.role, id, String(body?.status ?? ""));

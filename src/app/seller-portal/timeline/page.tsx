// ============================================================================
// 🏡 ZONO — Seller Portal — CLIENT TIMELINE (Client Experience 2.0). PHASE 56.0.
// Unified, live timeline + notification center. Composes the seller portal's own
// authenticated getters (RLS-scoped) — no new queries, isolation inherited.
// ============================================================================
import { getSellerExperience } from "@/lib/client-experience";
...
export default async function SellerTimelinePage() {
  const r = await getSellerExperience();
  if (r.state !== "ready") return <AuthGate state={r.state} email={r.state === "unlinked" ? r.email : null} />;
  const exp = r.data;

  return (
    <>
      <PortalNav active="/seller-portal/timeline" />
      <h1 className="text-2xl font-black text-slate-900">המסלול שלי</h1>
      <p className="mt-1 text-[13px] text-slate-500">כל מה שקורה עם הנכס שלך במקום אחד — שיווק, פגישות, מסמכים והצעות.</p>
      ...
      <section className="mt-6">
        ...
        {!exp.hasData ? <EmptyState title="אין עדיין פעילות על הנכס" body="פעילות שיווק, פגישות ומסמכים יופיעו כאן ברגע שיתחילו." /> : <Glass className="p-4"><ul>{exp.timeline.map((t) => <TimelineRow key={t.id} t={t} />)}</ul></Glass>}
      </section>

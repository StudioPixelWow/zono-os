...
import { getPropertyById, listPropertyMedia, type PropertyRow } from "@/lib/properties/repository";
import { propertyLocation } from "@/lib/properties/labels";
import type { PropertyInput } from "@/lib/properties/types";
import { PropertyWizard } from "../../new/PropertyWizard";

export const dynamic = "force-dynamic";

/** Edit reuses the FULL property wizard (media upload/edit, sellers, audiences,
 *  marketing kit, every field) so create and edit have identical capability. */
function rowToInput(p: PropertyRow): PropertyInput {
  ...
  return {
    ...
    marketingAudiences: Array.isArray(p.marketing_audiences) ? (p.marketing_audiences as string[]) : [],
...
export default async function EditPropertyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <PropertyWizard draftId={id} initial={rowToInput(p)} initialMedia={media} editMode />;
}

            ...
            <div key={m.id} className="border-line bg-surface rounded-xl border px-3 py-2 text-[12px]">
              ...
              {!["DONE", "CANCELLED"].includes(m.status) && (
                <div className="mt-1 flex gap-1">
                  {NEXT[m.status] && <button onClick={() => advance(m, NEXT[m.status]!)} disabled={pending === m.id} className="bg-brand-strong rounded px-2 py-0.5 text-[10px] font-bold text-white disabled:opacity-60">{NEXT[m.status] === "DONE" ? "סמן כהושלם" : "התחל ביצוע"}</button>}
                  <button onClick={() => advance(m, "CANCELLED")} disabled={pending === m.id} className="border-line bg-card text-muted rounded border px-2 py-0.5 text-[10px] font-bold disabled:opacity-60">בטל</button>
                </div>
              )}
            </div>
          ))}

...
function LeaderList({ title, rows, positive }: { title: string; rows: GrowthRow[]; positive: boolean }) {
  return (
    <div className="rounded-2xl border border-line bg-surface p-4">
      <h4 className="mb-2 flex items-center gap-1.5 text-sm font-black text-ink">
        <Icon name={positive ? "TrendingUp" : "TrendingDown"} size={15} className={positive ? "text-emerald-700" : "text-rose-700"} />{title}
      </h4>
        ...
        <div className="grid gap-1.5">
          {rows.map((r, i) => (
            <div key={`${r.key}_${i}`} className="flex items-center justify-between gap-2 text-sm">
              <span className="truncate text-ink">{i + 1}. {r.label}{r.city ? ` · ${r.city}` : ""}</span>
              <span className="flex items-center gap-2 text-xs text-muted"><Badge tone={positive ? "green" : "red"}>{r.deltaPct > 0 ? "+" : ""}{r.deltaPct}%</Badge>{r.prev}→{r.curr}</span>
            </div>
          ))}
        </div>
      )}
...
export function EvolutionView() {
  ...
  if (!data) {
    return <section dir="rtl" className="rounded-3xl border border-line bg-surface p-6 text-center text-sm text-muted">טוען מנוע אבולוציה…</section>;
  }
  ...
  return (
    <section dir="rtl" className="flex flex-col gap-5">
      <div className="relative overflow-hidden rounded-3xl border border-line bg-gradient-to-br from-[#0c1a2e] via-[#111d40] to-[#0a1429] p-6">
      ...
      {tab === "timemachine" && (
        <div className="flex flex-col gap-4">
          <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-line bg-surface p-4">
            <Icon name="Clock" size={18} className="text-cyan-700" />
            <span className="text-sm font-bold text-muted">הצג את תמונת השוק נכון לתאריך:</span>
            <input type="date" value={tmDate} max={new Date().toISOString().slice(0, 10)} onChange={(e) => setTmDate(e.target.value)}
              className="rounded-xl border border-line bg-surface px-3 py-1.5 text-sm text-ink [color-scheme:dark]" />
          ...
          {!tm ? <Empty text={pending ? "משחזר את ההיסטוריה…" : "בחר תאריך ולחץ 'הצג' כדי לשחזר את תמונת השוק כפי שהייתה."} /> : (
            <div className="flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                <Stat label="נכון לתאריך" value={tm.date} tone="text-cyan-700" />
                <Stat label="משרדים פעילים" value={tm.market.offices} />
                <Stat label="סוכנים פעילים" value={tm.market.agents} />
              </div>
              <div className="rounded-2xl border border-line bg-surface p-4">
                <h4 className="mb-2 text-sm font-black text-ink">משרדים מובילים (לפי מלאי) באותו זמן</h4>
                {tm.market.topOffices.length === 0 ? <Empty text="אין נתונים לתאריך זה." /> : (
                  <div className="grid gap-1.5">
                    {tm.market.topOffices.map((o, i) => (
                      <div key={i} className="flex items-center justify-between gap-2 text-sm">
                        <span className="truncate text-ink">{i + 1}. {o.label}{o.city ? ` · ${o.city}` : ""}</span>
                        <span className="text-xs text-muted">{o.listings} מודעות</span>
                      </div>
                    ))}
                  </div>
                )}
      ...
      {tab === "neighborhoods" && (
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          ...
          {data.neighborhoodLeaders.map((nb, i) => (
            <div key={`${nb.city}_${nb.neighborhood}_${i}`} className="rounded-2xl border border-line bg-surface p-4">
      ...
      {tab === "market" && (
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          ...
          {data.marketDna.map((m, i) => (
            <div key={`${m.city}_${i}`} className="rounded-2xl border border-line bg-surface p-4">
              <div className="flex items-center justify-between"><span className="text-sm font-black text-ink">{m.city}</span><Badge tone="violet">{m.dominantOfficeCategory}</Badge></div>
              <p className="mt-2 text-[11px] text-muted">סוג נכס מוביל: {m.dominantPropertyCategory} · עצמת תחרות {m.competitionIntensity}%</p>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          {data.predictions.map((p) => (
            <div key={p.id} className="rounded-2xl border border-line bg-surface p-4">
              <div className="flex items-start justify-between gap-2">
                <div className="text-sm font-black text-ink">{PRED_HE[p.predictionType] ?? p.predictionType}{p.city ? <span className="text-muted"> · {p.city}</span> : null}</div>
              ...
              {p.explanation && <p className="mt-2 text-[11px] text-violet-700">{p.explanation}</p>}
              <p className="mt-1 text-[10px] text-muted/70">הערכה בלבד — לא ודאות.</p>
            </div>
          ))}
        </div>
      )}

      <p className="text-[11px] text-muted/70">מנוע אבולוציה — חישוב דטרמיניסטי על בסיס היסטוריה מצטברת. תחזיות אינן עובדות. מידע ציבורי/עסקי בלבד · ללא מחיקה אוטומטית · הסבר לכל פלט.</p>
    </section>
  );
}

...
function Badge({ children, tone = "white" }: { children: React.ReactNode; tone?: "white" | "green" | "amber" | "red" | "violet" }) {
  const c = tone === "green" ? "bg-emerald-50 text-emerald-700" : tone === "amber" ? "bg-amber-50 text-amber-700" : tone === "red" ? "bg-rose-50 text-rose-700" : tone === "violet" ? "bg-violet-500/15 text-violet-200" : "bg-surface text-muted";
...
function Stat({ label, value, tone }: { label: string; value: number | string; tone?: string }) {
  return <div className="rounded-2xl border border-line bg-surface p-4"><div className={`text-2xl font-black ${tone ?? "text-ink"}`}>{value}</div><div className="mt-1 text-xs font-bold text-muted">{label}</div></div>;
}
function Empty({ text }: { text: string }) { return <div className="rounded-2xl border border-dashed border-line bg-surface p-6 text-center text-sm text-muted">{text}</div>; }
...
export function KnowledgeView() {
  ...
  if (!data) {
    return <section dir="rtl" className="rounded-3xl border border-line bg-surface p-6 text-center text-sm text-muted">טוען שכבת ידע…</section>;
  }
  ...
  return (
    <section dir="rtl" className="flex flex-col gap-5">
      <div className="relative overflow-hidden rounded-3xl border border-line bg-gradient-to-br from-[#150c2e] via-[#1d1140] to-[#120829] p-6">
      ...
      <div className="flex flex-wrap gap-2">
        {tabs.filter((t) => !t.owner || owner).map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)} className={`rounded-xl px-3 py-1.5 text-sm font-bold transition ${tab === t.id ? "bg-brand-strong text-ink" : "border border-line bg-surface text-muted hover:text-ink"}`}>{t.label}</button>
        ))}
      </div>
      ...
      {tab === "health" && owner && (
        data.health ? (
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <Stat label="ציון בריאות" value={`${data.health.healthScore}%`} tone={data.health.healthScore >= 70 ? "text-emerald-700" : "text-amber-700"} />
            <Stat label="רשומות תקינות" value={data.health.healthy} tone="text-emerald-700" />
            <Stat label="לבדיקה" value={data.health.needsReview} tone="text-amber-700" />
            <Stat label="ביטחון נמוך" value={data.health.lowConfidence} tone="text-amber-700" />
            <Stat label="חסר טלפון" value={data.health.missingPhones} />
            <Stat label="חסר אימייל" value={data.health.missingEmails} />
            <Stat label="אשכולות כפילות" value={data.health.duplicateClusters} tone="text-rose-700" />
            <Stat label="כיסוי ממוצע" value={`${data.health.coveragePct}%`} tone="text-violet-700" />
          </div>
      ...
      {tab === "completeness" && (
        <div className="grid gap-3 md:grid-cols-2">
          ...
          {data.completenessLow.map((c) => (
            <div key={`${c.entityType}_${c.entityId}`} className="rounded-2xl border border-line bg-surface p-4">
              <div className="flex items-center justify-between text-xs text-muted"><span>{c.entityType === "office" ? "משרד" : "סוכן"} · {c.city ?? "—"}</span><span className="font-black text-ink">{c.pct}%</span></div>
              <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-surface"><div className={`h-full ${bar(c.pct)}`} style={{ width: `${c.pct}%` }} /></div>
              ...
              {c.suggestions.length > 0 && <p className="mt-2 text-[11px] text-violet-700">המלצה: {c.suggestions[0]}</p>}
              <button onClick={() => openGraph(c.entityType as "office" | "agent", c.entityId, c.city ?? "")} className="mt-2 text-[11px] font-bold text-muted hover:text-ink">פתח בגרף ←</button>
            </div>
          ))}
        </div>
      )}
          ...
          {data.clusters.map((c) => (
            <div key={c.id} className="rounded-2xl border border-line bg-surface p-4">
              <div className="flex items-start justify-between gap-2">
                <div className="text-sm font-black text-ink">{c.entityType === "office" ? "משרד" : "סוכן"} · {c.memberCount} רשומות · {c.city ?? "—"}</div>
                <Badge tone={c.confidence >= 90 ? "green" : c.confidence >= 70 ? "amber" : "red"}>{Math.round(c.confidence)}% · {REC_HE[c.recommendation ?? ""] ?? ""}</Badge>
              </div>
              {c.explanation && <p className="mt-1 text-xs text-violet-700">{c.explanation}</p>}
      ...
      {tab === "market" && (
        <div className="grid gap-3 md:grid-cols-2">
          ...
          {["office", "network", "city"].map((scope) => {
            ...
            return (
              <div key={scope} className="rounded-2xl border border-line bg-surface p-4">
                <h4 className="mb-2 text-sm font-black text-ink">{scope === "office" ? "משרדים מובילים" : scope === "network" ? "רשתות מובילות" : "מובילי ערים"}</h4>
                <div className="grid gap-1.5">
                  {rows.map((r, i) => (
                    <div key={`${r.scopeKey}_${i}`} className="flex items-center justify-between gap-2 text-sm">
                      <span className="truncate text-ink">{i + 1}. {r.scopeLabel ?? r.scopeKey}{r.city ? ` · ${r.city}` : ""}</span>
                      <span className="flex items-center gap-2 text-xs text-muted"><span className="font-bold text-violet-700">{r.sharePct}%</span>{r.listings} מודעות</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {tab === "coverage" && (
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          ...
          {data.coverage.map((c) => (
            <div key={c.city} className="rounded-2xl border border-line bg-surface p-4">
              <div className="flex items-center justify-between"><span className="text-sm font-black text-ink">{c.city}</span><Badge tone={c.coveragePct >= 70 ? "green" : "amber"}>{c.coveragePct}%</Badge></div>
              <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-surface"><div className={`h-full ${bar(c.coveragePct)}`} style={{ width: `${c.coveragePct}%` }} /></div>
              <p className="mt-2 text-[11px] text-muted">ידועים: {c.knownOffices} משרדים · {c.knownAgents} סוכנים · חסרים ~{c.missingOffices} · ביטחון {c.confidence}%</p>
            </div>
          ))}
        </div>
      )}
      ...
      {tab === "discoveries" && (
        <div className="grid gap-3 md:grid-cols-2">
          ...
          {data.discoveries.map((d) => (
            <div key={d.id} className="rounded-2xl border border-line bg-surface p-4">
              <div className="flex items-start justify-between gap-2"><div className="text-sm font-black text-ink">{DISC_HE[d.discoveryType] ?? d.discoveryType}</div><Badge tone={d.confidence >= 80 ? "green" : "amber"}>{Math.round(d.confidence)}%</Badge></div>
              <p className="mt-1 text-xs text-muted">{d.reasons.join(" · ") || "—"}{d.city ? ` · ${d.city}` : ""}</p>
              {d.aiExplanation && <p className="mt-1 text-[11px] text-violet-700">{d.aiExplanation}</p>}
              {owner && (
                <div className="mt-3 flex gap-2">
      ...
      <p className="text-[11px] text-muted/70">שכבת ידע — חישוב דטרמיניסטי, מקור אמת יחיד, הסבר לכל החלטה. מידע ציבורי/עסקי בלבד · אין מחיקה אוטומטית.</p>
    </section>
  );
}
...
function GraphMini({ graph, onExpand }: {
  ...
  return (
    <div>
      <div className="mb-2 text-sm font-black text-ink">קשרים סביב: {center.label} <span className="text-muted">({neighbors.length} צמתים)</span></div>
      ...
      <p className="text-[11px] text-muted">לחיצה על משרד/סוכן מרחיבה את הגרף סביבו.</p>
    </div>
  );
}

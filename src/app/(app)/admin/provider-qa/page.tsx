...
import { getProviderQADashboardAction } from "@/lib/property-radar/provider-qa/actions";
import { ProviderQAView } from "./ProviderQAView";

export const dynamic = "force-dynamic";

export default async function ProviderQAPage() {
  const res = await getProviderQADashboardAction();
  if (!res.ok) {
    return (
      <div className="bg-card border-line m-4 flex flex-col items-center gap-3 rounded-[20px] border p-10 text-center">
        ...
        <p className="text-muted text-sm">בקרת איכות הספקים זמינה למנהלים בלבד.</p>
        <Link href="/" className="text-brand-strong text-sm font-bold">חזרה לדשבורד</Link>
      </div>
    );
  }
  return <ProviderQAView initial={res.data} />;
}

...
export default async function OfficeSitePropertiesPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-white text-[#0f172a]">
      <nav className="border-b border-[#eef0f4] bg-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
          <Link href={`/site/${slug}`} className="font-black text-[#1e1b4b]">← חזרה לאתר</Link>
          <h1 className="text-lg font-black">כל הנכסים</h1>
        </div>
      </nav>
      <main className="mx-auto max-w-6xl px-4 py-8">
        {props.length === 0 ? (
          <p className="py-16 text-center text-[#64748b]">אין נכסים זמינים כרגע.</p>
        ) : (
...
function Card({ p }: { p: PublicProperty }) {
  return (

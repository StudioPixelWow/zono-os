...
-- No new queue, no parallel state machine. Reuses the EXISTING (previously
-- code-less) distribution_publish_events (append-only audit, target_id→posts)
-- and distribution_publish_controls (emergency stop). Ports the content_hash
-- duplicate-post guarantee off the parallel distribution_group_posts.
--
...
-- Legacy rows: assign to their creator so agent-isolated claiming has an owner.
...
-- Callback de-duplication: a provider/extension callback is applied at most once.
...
-- Enforces org scope, agent (user) isolation, emergency-stop (org + per-scope),
-- schedule due, not paused/terminal, and lease freedom. Uses FOR UPDATE SKIP
-- LOCKED so concurrent workers/instances NEVER receive the same post.
...
declare
  ...
  v_from text;
begin
  -- Org-wide emergency stop blocks all serving.
  if exists (
  ...
  where p.org_id = p_org
    and coalesce(p.assigned_user_id, p_user) = p_user            -- agent isolation
    ...
    and not exists (                                             -- scoped emergency stop
      select 1 from public.distribution_publish_controls c
      where c.org_id = p_org and c.state = 'active'
